...
static int queue_setup(struct vb2_queue *vq, unsigned int *nbuffers, unsigned int *nplanes, unsigned int sizes[], struct device *alloc_ctxs[])
{
	...
	size = F_DATA.width * F_DATA.height << 1;
	if (!get_frame()) {
		v4l2_err(&dev->v4l2_dev, "%s: v4l2 is not initialized..\n", __func__);
		return -ENODATA;
	}
...
static const struct vb2_ops yuv_qops = {
...
static const struct v4l2_ioctl_ops yuv_ioctl_ops = {
...
static int p_probe(struct platform_device *pdev)
{
	...
	q->drv_priv = dev;
	q->buf_struct_size = sizeof(struct yuv_buffer);
	q->ops = &yuv_qops;
...
MODULE_DESCRIPTION("V4L2 Driver");
MODULE_LICENSE("GPL");
